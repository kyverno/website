...
Note that Kyverno retries up to three times to reconcile a `Failed` `UpdateRequest`, and the `UpdateRequest` will be garbage collected if it exceeds the retry threshold.
