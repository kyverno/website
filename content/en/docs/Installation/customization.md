...
60. `imageVerifyCacheEnabled` (A): specifies wheather the verifyImage cache is enable or not. Defaults is `true`. <br>
61. `imageVerifyCacheMaxSize` (A): specifies the max size limit for the verifyImage cache. Defaults is `1000`. <br>
62. `imageVerifyCacheTTLDuration` (A): specifies the TTL value for verifyImage cache. Defaults is `60m`. (Here the ttl values should be entered in minutes.) <br>
