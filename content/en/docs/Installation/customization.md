...
23. `genWorkers` (B): the number of workers for processing generate policies concurrently. Default is `10`.
24. `imagePullSecrets` (ABR): specifies secret resource names for image registry access credentials. Only a single value accepted currently.
25. `imageSignatureRepository` (AR): specifies alternate repository for image signatures. Can be overridden per rule via `verifyImages.Repository`.
26. `imageVerifyCacheEnabled` (A): Enable a TTL cache for verified images. Default is `true`.
27. `imageVerifyCacheMaxSize` (A): Maximum number of keys that can be stored in the TTL cache. Keys are a combination of policy elements along with the image reference. Default is `1000`. `0` sets the value to default.
28. `imageVerifyCacheTTLDuration` (A): Maximum TTL value for a cache expressed as duration. Default is `60m`. `0` sets the value to default.
29. `kubeconfig` (ABCR): specifies the Kubeconfig file to be used when overriding the API server to which Kyverno should communicate. Only used when Kyverno is running outside of the cluster in which it services admission requests.
30. `leaderElectionRetryPeriod` (ABCR): controls the leader election renewal frequency. Default is `2s`.
31. `log_backtrace_at` (ABCR): when logging hits line file:N, emit a stack trace.
32. `log_dir` (ABCR): if non-empty, write log files in this directory (no effect when -logtostderr=true).
33. `log_file` (ABCR): if non-empty, use this log file (no effect when -logtostderr=true).
34. `log_file_max_size` (ABCR): defines the maximum size a log file can grow to (no effect when -logtostderr=true). Unit is megabytes. If the value is 0, the maximum file size is unlimited. Default is `1800`.
35. `loggingFormat` (ABCR): determines the output format of logs. Logs can be outputted in JSON or text format by setting the flag to `json` or `text` respectively. Default is `text`.
36. `logtostderr` (ABCR): log to standard error instead of files. Default is `true`.
37. `maxQueuedEvents` (ABR): defines the upper limit of events that are queued internally. Default is `1000`.
38. `metricsPort` (ABCR): specifies the port to expose prometheus metrics. Default is `8000`.
39. `omit-events` (ABR): specifies the type of Kyverno events which should not be emitted. Accepts a comma-separated string with possible values `PolicyViolation`, `PolicyApplied`, `PolicyError`, and `PolicySkipped`. Default is undefined (all events will be emitted).
40. `one_output` (ABCR): If true, only write logs to their native severity level (vs also writing to each lower severity level; no effect when -logtostderr=true).
41. `otelCollector` (ABCR): sets the OpenTelemetry collector service address. Kyverno will try to connect to this on the metrics port. Default is `opentelemetrycollector.kyverno.svc.cluster.local`.
42. `otelConfig` (ABCR): sets the preference for Prometheus or OpenTelemetry. Set to `grpc` to enable OpenTelemetry. Default is `prometheus`.
43. `policyReports` (R): enables the Policy Reports system (1.10.2+). When enabled, Policy Report Custom Resources will be generated and managed in the cluster. Default is `true`.
44. `profile` (ABCR): setting this flag to `true` will enable profiling. Default is `false`.
45. `profileAddress` (ABCR): Configures the address of the profiling server. Default is `""`.
46. `profilePort` (ABCR): specifies port to enable profiling. Default is `6060`.
47. `protectManagedResources` (A): protects the Kyverno resources from being altered by anyone other than the Kyverno Service Account. Defaults to `false`. Set to `true` to enable.
48. `registryCredentialHelpers` (ABR): enables cloud-registry-specific authentication helpers. Defaults to `"default,google,amazon,azure,github"`.
49. `reportsChunkSize` (R): maximum number of results in generated reports before splitting occurs if there are more results to be stored. Default is `1000`.
50. `serverIP` (AC): like the `kubeconfig` flag, used when running Kyverno outside of the cluster which it serves.
51. `servicePort` (AC): port used by the Kyverno Service resource and for webhook configurations. Default is `443`.
52. `skipResourceFilters` (R): defines whether to obey the ConfigMap's resourceFilters when performing background report scans. Default is `true`. When set to `true`, anything defined in the resourceFilters will not be excluded in background reports. Ex., when set to `true` if the resourceFilters contain the `[*/*,kube-system,*]` entry then background scan reports will be produced for anything in the `kube-system` Namespace. Set this value to `false` to obey resourceFilters in background scan reports. Ex., when set to `false` if the resourceFilters contain the `[*/*,kube-system,*]` entry then background scan reports will NOT be produced for anything in the `kube-system` Namespace.
53. `skip_headers` (ABCR): if true, avoid header prefixes in the log messages.
54. `skip_log_headers` (ABCR): if true, avoid headers when opening log files (no effect when -logtostderr=true).
55. `stderrthreshold` (ABCR): logs at or above this threshold go to stderr when writing to files and stderr (no effect when -logtostderr=true or -alsologtostderr=false). Default is `2`.
56. `tlsSecretName` (AC): overwrites the default secret name of the TLS certificate. See also the related flag `caSecretName`.
57. `tracingAddress` (ABCR): tracing receiver address, defaults to `''`.
58. `tracingCreds` (ABCR): set to the CA secret containing the certificate which is used by the Opentelemetry Tracing Client. If empty string is set, an insecure connection will be used.
59. `tracingPort` (ABCR): tracing receiver port. Default is `"4317"`.
60. `transportCreds` (ABCR): set to the CA secret containing the certificate used by the OpenTelemetry metrics client. Empty string means an insecure connection will be used. Default is `""`.
61. `ttlReconciliationInterval` (C): defines the interval the cleanup controller should perform reconciliation of resources labeled with the cleanup TTL label. Default is `1m`. See the cleanup documentation [here](/docs/writing-policies/cleanup/#cleanup-label) for details.
62. `tufMirror` (AR): specifies alternate TUF mirror for sigstore. If left blank, public sigstore one is used for cosign verification.
63. `tufRoot` (AR): specifies alternate TUF root.json for sigstore. If left blank, public sigstore one is used for cosign verification.
64. `v` (ABCR): sets the verbosity level of Kyverno log output. Takes an integer from 1 to 6 with 6 being the most verbose. Level 4 shows variable substitution messages. Default is `2`.
65. `vmodule` (ABCR): comma-separated list of pattern=N settings for file-filtered logging.
66. `webhookRegistrationTimeout` (A): specifies the length of time Kyverno will try to register webhooks with the API server. Defaults to `120s`.
67. `webhookTimeout` (A): specifies the timeout for webhooks, in seconds. After the timeout passes, the webhook call will be ignored or the API call will fail based on the failure policy. The timeout value must be an integer number between 1 and 30 (seconds). Defaults is `10`.
