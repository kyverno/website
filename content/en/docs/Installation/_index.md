...
Kyverno is installable using Helm or deploying from the manifest directly. When using either of these methods, there are no other steps required to get Kyverno up and running. 

{{% alert title="Note" color="info" %}}
Currently, Kyverno only supports a single replica in the `Deployment` resource.
{{% /alert %}}
