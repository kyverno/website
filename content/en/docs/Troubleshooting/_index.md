...
5. Have you checked the number of pending update or generate requests? In addition to the admission request count metric, you can use `kubectl -n kyverno get updaterequests` or `kubectl -n kyverno get generaterequests` to get a sense of the request count. If there are a lot of Pending requests this could be a sign of a permissions issue - are you granting the controller clusterrroles permissions to the resources and operations it needs?
