---
title: "Require QoS Guaranteed in ValidatingPolicy"
...
metadata:
  ...
  annotations:
    policies.kyverno.io/title: Require QoS Guaranteed in ValidatingPolicy
...
spec:
  ...
  validations:
          - expression: >-
              ...
              container.resources.?requests.orValue({}).?cpu.hasValue() &&
              container.resources.?requests.orValue({}).?memory.hasValue() &&
              container.resources.?limits.orValue({}).?cpu.hasValue() &&
              container.resources.?limits.orValue({}).?memory.hasValue() &&
              container.resources.requests.orValue({}).cpu == container.resources.limits.orValue({}).cpu &&
              container.resources.requests.orValue({}).memory == container.resources.limits.orValue({}).memory)
