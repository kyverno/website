...
:::note[Private Registry Access]
If cleanup policies inspect images in private container registries (for example, via image metadata expressions), the cleanup controller can authenticate to private registries using `--imagePullSecrets` and `--registryCredentialHelpers`. See [Container Flags](/docs/installation/customization#container-flags) for details.
:::
