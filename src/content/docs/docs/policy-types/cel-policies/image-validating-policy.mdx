...
spec:
  ...
  variables:
    - name: cm
      expression: 'resource.Get("v1", "configmaps", namespaceObject.metadata.name, "keys")'
  matchImageReferences:
    - glob: ghcr.io/*
  attestors:
    - name: notary
