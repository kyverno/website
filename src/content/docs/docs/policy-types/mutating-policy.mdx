...
As of kyverno 1.17, its possible to select mutation targets with a CEL expression. The expression must use the resource library because it must evaluate to a kubernetes object. The expression can be defined under the `targetMatchConstraints.expression` field with takes a string CEL expression.
