...
To allow `image.GetMetadata()` to access private container registries, credentials can be supplied to Kyverno controllers using the `--imagePullSecrets` and `--registryCredentialHelpers` container flags. This registry credential support is available across Kyverno controllers, including the admission controller, background controller, reports controller, and cleanup controller.
:::
