...
spec:
  ...
  validations:
    - expression: >-
        images.imagerefs.map(image, verifyImageSignatures(image, [attestors.cosign])).all(e ,e > 0)
      message: >-
        ...
        certificate:
          ...
          certChain:
            value: |
              ...
              ...
              -----END CERTIFICATE-----
            expression: variables.cm.data.certChain
...
This policy demonstrates how to use a custom image extractor to verify images in a custom resource where the image reference is not in a standard location.

The `spec.images` field takes a list of named CEL expressions to determine where the images are located within the resource. The extracted images can then be referenced in the `validations` expressions using the given name (e.g., `images.custom_image`).

For example, given a custom resource `myresources.example.io` that defines an image in `foo.bar`:
...
foo:
  bar: 'ghcr.io/myrepo/myapp:v1.0.0'
```

The following policy extracts and verifies this image:

```yaml
apiVersion: policies.kyverno.io/v1
kind: ImageValidatingPolicy
metadata:
  name: verify-custom-resource-image
spec:
  webhookConfiguration:
    timeoutSeconds: 15
  evaluation:
  ...
  validations:
    - expression: >-
        images.custom_image.map(image, verifyImageSignatures(image, [attestors.cosign])).all(e ,e > 0)
      message: >-
        failed the image Signature verification
```
