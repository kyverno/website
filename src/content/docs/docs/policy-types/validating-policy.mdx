...
## HTTP and Envoy Authorization Payloads

The Kyverno `ValidatingPolicy` can be used to evaluate HTTP and Envoy External Authorization requests. This allows policies to act as an authorization policy engine for HTTP-based and Envoy-based workloads.

### HTTP Authorization

Here is an example of a policy that evaluates an HTTP authorization request:
...
To write a policy for HTTP authorization payloads:

1. Set `spec.evaluation.mode` to `HTTP`.
2. Use the `http` CEL library in `spec.validations` to define your authorization logic.

The corresponding HTTP check request payload (`request.json`):
...
To evaluate HTTP authorization policies, use the [Kyverno CLI](/docs/subprojects/kyverno-cli) with the `--http-payload` flag:
...
Here is an example of a policy that evaluates an Envoy External Authorization `CheckRequest`:
...
To write a policy for Envoy authorization payloads:

1. Set `spec.evaluation.mode` to `Envoy`.
2. Use the `envoy` CEL library in `spec.validations` to define your authorization logic.

The corresponding Envoy check request payload (`envoy-request.json`):
...
To evaluate Envoy authorization policies, use the [Kyverno CLI](/docs/subprojects/kyverno-cli) with the `--envoy-payload` flag:
