...
| `exemplarFilter` (ABCR)                | `trace-based`                                               | Configures the OpenTelemetry exemplar filter for metrics. Exemplars are sample data points attached to aggregated metrics that link back to specific trace spans, enabling correlation between metrics and traces. Supported values: `off` (no exemplars collected), `trace-based` (exemplars collected only when the context has a sampled trace span), `always-on` (exemplars collected on every measurement). Applies to both Prometheus and OTLP gRPC metrics backends.                                                                                                                                                                                       |
