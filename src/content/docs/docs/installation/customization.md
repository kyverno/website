...
| `exemplarFilter` (ABCR)                 | `trace-based`                                       | Configures the [OpenTelemetry exemplar](https://opentelemetry.io/docs/specs/otel/metrics/sdk/#exemplar) filter for metrics. Exemplars are sample data points attached to aggregated metrics (such as histograms and counters) that link back to specific trace spans, enabling correlation between metrics and traces. Supported values: `always-off` (no exemplars collected), `trace-based` (exemplars collected only when the context has a sampled trace span), `always-on` (exemplars collected on every measurement). Applies to both Prometheus and OTLP gRPC metrics backends. |
...
| `metricsCASecretName` (ABCR)            |                                                     | Name of the Kubernetes Secret containing the CA certificate used to sign the metrics TLS certificate. Used together with `--metricsTLS`. See [TLS for the /metrics endpoint](/docs/guides/monitoring#tls-for-the-metrics-endpoint) for details.                                                                                                                                                                                                                                                                                                                                                                                                                |
| `metricsKeyAlgorithm` (ABCR)            | `RSA`                                               | Key algorithm for auto-generated self-signed metrics TLS certificates. Supported values: `RSA`, `ECDSA`, `Ed25519`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `metricsRenewBefore` (ABCR)             | `360h0m0s`                                          | How long before expiry the metrics TLS certificate is automatically renewed. Uses Go duration format, e.g. `360h0m0s` for 15 days.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `metricsServerIP` (ABCR)                |                                                     | IP address where the metrics server is reachable. Used to add IP Subject Alternative Names (SANs) to the auto-generated certificate. Required when accessing the metrics endpoint outside the cluster by IP address.                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `metricsTLS` (ABCR)                     | `false`                                             | Enables TLS on the metrics HTTP server, serving the `/metrics` endpoint over HTTPS. See [TLS for the /metrics endpoint](/docs/guides/monitoring#tls-for-the-metrics-endpoint) for details.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `metricsTLSSecretName` (ABCR)           |                                                     | Name of the Kubernetes Secret containing the TLS certificate and key for the metrics server. If not set and `--metricsTLS` is enabled, Kyverno generates a self-signed certificate automatically.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
