...
The `apply` command supports testing HTTP and Envoy External Authorization policies locally without a running cluster, using the `--http-payload` and `--envoy-payload` flags respectively. These flags accept a path to a JSON file representing the authorization check request.

Apply an HTTP authorization policy against an HTTP check request payload:

```sh
kyverno apply /path/to/policy.yaml --http-payload /path/to/request.json
```

Apply an Envoy authorization policy against an Envoy check request payload:

```sh
kyverno apply /path/to/policy.yaml --envoy-payload /path/to/request.json
```

Multiple payloads can be passed by repeating the flag:

```sh
kyverno apply /path/to/policy.yaml --envoy-payload /path/to/request1.json --envoy-payload /path/to/request2.json
```

Both flags can be used together when applying multiple policies of different types:

```sh
kyverno apply /path/to/http-policy.yaml /path/to/envoy-policy.yaml --http-payload /path/to/http-request.json --envoy-payload /path/to/envoy-request.json
```

#### HTTP Authorization Policies — Example

Policy manifest (`http-authz-policy.yaml`):
...
HTTP check request payload (`request.json`):
...
Apply the HTTP authorization policy against the request payload:

```sh
kyverno apply http-authz-policy.yaml --http-payload request.json
```

#### Envoy External Authorization Policies — Example

Policy manifest (`envoy-authz-policy.yaml`):
...
Envoy check request payload (`envoy-request.json`):
...
Apply the Envoy authorization policy against the request payload:

```sh
kyverno apply envoy-authz-policy.yaml --envoy-payload envoy-request.json
```

Both flags can be combined in a single invocation when testing multiple policy types simultaneously:

```sh
kyverno apply http-authz-policy.yaml envoy-authz-policy.yaml --http-payload request.json --envoy-payload envoy-request.json
```
...
9. The `globalContextEntries` element which declares mock data for `GlobalContextEntry` references, satisfying both `context.globalReference` (v1 ClusterPolicy) and `globalContext.Get()` (CEL ValidatingPolicy) calls offline. Each entry supports three mutually exclusive data sources: `data` (arbitrary JSON value), `resources` (inline Kubernetes resource manifests), or `resourceFiles` (paths to YAML files relative to the test directory). The `resources` and `resourceFiles` options decode manifests into the same `[]interface{}` shape as the real Kubernetes resource cache, making it possible to test `kubernetesResource`-backed GlobalContextEntries fully offline. Optional.
