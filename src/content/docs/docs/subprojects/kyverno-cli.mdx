...
globalContextEntries: # optional; inline mock data for GlobalContextEntry references (offline testing)
  - name: <globalcontextentry-name>
    ...
    fieldPath: '' # optional JMESPath to narrow the root
...
9. The `globalContextEntries` element which declares inline mock data for `GlobalContextEntry` references, satisfying both `context.globalReference` (v1 ClusterPolicy) and `globalContext.Get()` (CEL ValidatingPolicy) calls offline. Optional.
...
globalContextEntries:
  - name: gctxentry-deployments # Must match the GlobalContextEntry name used in the policy
    data: # Root JSON object (arbitrary structure)
      deployments:
        - name: existing-deployment-1
        - name: existing-deployment-2
    fieldPath: deployments # Optional JMESPath applied to `data` before projections
