...
In Kyverno 1.18, the `--exceptions-with-policies` flag lets you bundle `PolicyException` resources alongside their associated policy files, rather than passing them separately with the `--exception` flag. This is convenient for organizing exceptions co-located with policies in the same directory.
...
Starting with Kyverno 1.18, `kyverno apply` accepts `CleanupPolicy` and `ClusterCleanupPolicy` resources. When applied, the CLI performs a local dry-run evaluation of the cleanup rules against the provided resources. It does **not** call the Kubernetes delete API. The evaluation checks namespace matching, `match`/`exclude` conditions, and any `conditions` defined in the policy.
...
Kyverno 1.18 adds support for testing `MutatingPolicy` resources that use `mutateExisting: true` rules — rules that mutate resources already present in the cluster (rather than the incoming admission request). When the `mutateExisting` flag is set on a rule, the rule is triggered by a separate **trigger** resource and mutates one or more **target** resources.
...
Kyverno 1.18 adds support for testing `ValidatingPolicy` resources that use the `HTTP` or `Envoy` evaluation modes via the `kyverno test` command. These policies validate HTTP or Envoy authorization check requests rather than Kubernetes resources.
