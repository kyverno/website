...
For classic `verifyImages` rules, background-scan reports do not include the `verifyDigest` check from the admission validation webhook. As a result, an existing resource that uses a tag-only image reference is not reported as a digest violation by the background scan. Background scanning also does not persist `mutateDigest` changes to existing resources. These checks and mutations apply when a resource is admitted; use a separate validation rule if existing resources must be reported for digest pinning.
