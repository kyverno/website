...
| kyverno apply shows Applying 0 policy rule(s) or spec.rules: ... unknown field | Kyverno CLI apply currently parses Policy/ClusterPolicy schemas and does not execute ValidatingPolicy offline | Validate ValidatingPolicy on a cluster with kubectl apply, or use a supported local test workflow if/when available |
