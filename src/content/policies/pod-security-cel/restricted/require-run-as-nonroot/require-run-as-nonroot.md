...
metadata:
  ...
  annotations:
    ...
    policies.kyverno.io/description: Containers must be required to run as non-root. This policy ensures `runAsNonRoot` is set to true.
spec:
        ...
        cel:
          variables:
            ...
            - name: podRunAsNonRoot
              expression: variables.podSecurityContext.?runAsNonRoot.orValue(false)
            - name: podLevelEnforced
              expression: |-
                variables.podRunAsNonRoot && variables.allContainers.all(container,
                  container.?securityContext.?runAsNonRoot.orValue(true) == true
                )
            - name: containerLevelEnforced
              expression: |-
