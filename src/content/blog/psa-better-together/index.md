...
slug: using-kyverno-with-pod-security-admission
tags:
  - General
...
Take one of the earlier policies we've pointed out along the way and see a sample GitHub Action workflow we publish in the [documentation](/docs/guides/testing-policies#github-actions) and you're up and running. The `kyverno apply` command will naturally exit with an error code if the resource(s) do not pass. The CLI combined with Kyverno's rule auto-generation capability mentioned earlier means you can be informed of those Deployments, StatefulSets, and others before they need to touch a cluster.
